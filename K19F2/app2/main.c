...
void createAccount(struct Account* accountList, int* accountCount) {
    ...
    // Copy the username, password, and set permission level
    ...
    // Save the updated account list to file
   // saveAccounts(accountList, *accountCount);
...
void exitProgram(){
    ...
    if (choice == 1) {
        ...
        printf("    ###     ###    ###  ##         ##  ###     ####  ###    ###         ####        ####      ######## \n");
        printf("\e[0;37m");
        Sleep(2000); // Tạm dừng chương trình trong 3 giây
...
void adminmanagement (int*studentCount,int studentList,struct Account* accountList, int*accountCount){
    ...
    do {
                                    ...
                                    switch (choice) {
                                        ...
                                        case 3:
                                            printf("\n");
                                            viewAllAccounts(accountList, *accountCount);
                                            printf("\n");
                                            break;
                                        case 4:
                                            system("cls");
...
void viewAllAccounts(struct Account *accountList, int accountCount) {
    printf("All Accounts:\n");
    for (int i = 0; i < accountCount; ++i) {
...
void loadAccounts(struct Account *accountList, int *accountCount) {
    ...
    *accountCount = 0;
    while ((*accountCount) < MAX_ACCOUNTS && fscanf(file, "%s %s %d", accountList[*accountCount].username,accountList[*accountCount].password,&accountList[*accountCount].permissionLevel) != EOF) {
        (*accountCount)++;
    }
    fclose(file);
}
